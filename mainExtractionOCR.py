...
# import base64                           #Encodes images into Base64 (used for sending images to OpenAI API).
# import openai
...
# # Opens an image, reads it in binary mode, and encodes it as base64.
# # This is required to send images to OpenAI's API.
...
# # Encodes the extracted image in Base64
# # Prepares a prompt for GPT-4o-mini.
# # Sends metadata (extracted text) + image to GPT-4o-mini.
...
# # Ensures the output directories exist (to store processed images and metadata).
...
#         #Reads the uploaded image as a NumPy array.Converts it to grayscale (IMREAD_GRAYSCALE) for processing.
...
#     #    Extracts filename (used for saving processed data).
...
# #Creates structuring elements for line detection:
# #                     - horiz_kernel: Detects horizontal lines.
# #                     - verti_kernel: Detects vertical lines.    

#         horiz_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (1, ncol//150))  #Creates a rectangular structuring element (kernel).Height = 1 pixel (very thin).Width = ncol//150 pixels (adaptive based on image width).This makes the kernel long and horizontal.
